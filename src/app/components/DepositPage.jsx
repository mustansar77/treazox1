...
const DepositPage = () => {
  ...
  useEffect(() => {
    // Dummy current assets (replace with API)
    setAssets(12450);
  }, []);

  const handleDeposit = () => {
    if (!selectedAmount && !customAmount) {
      toast.error("Please select or enter deposit amount");
      return;
    }
    if (!trxId) {
    ...
    toast.success(`Deposited $${amount} via ${selectedExchange.name}`);
    // Reset fields
    ...
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen p-4 sm:p-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 mb-6">
        ...
        <div className="bg-gray-100 dark:bg-gray-900 p-4 rounded-lg mb-4">
          ...
          <p className="font-mono text-gray-700 dark:text-green-500">
