...
const DepositPage = () => {
    const [deposits, setDeposits] = useState([]);
      ...
      const filteredDeposits = deposits.filter((d) =>
        d.user?.email.toLowerCase().includes(searchEmail.toLowerCase())
      );
              ...
              filteredDeposits.map((item, index) => (
                <tr key={item._id}>
                  ...
                  <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-200">
                    ...
                    <span className="font-mono">{item.exchange?.address}</span>
                  </td>
                  <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-200 break-all">{item.trxId}</td>
